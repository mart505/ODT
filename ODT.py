...
dataset = "bc"              # choose from: mush, ttt, heloc, votes, student, bc, heart, monks-1, krkp
treeTopology = "depth3"     # choose from: depth2 depth2.5 depth3 imbalanced
trainingVSClass = 200       # choose from: 90%/600 or an integer value
...
corrHeatmap = True          # print feature selection information
...
def build_Tree(relaxation, strengthening, anchoring, numericalFeature):
    ...
    if (relaxation) :
        ...
        c = mdl.continuous_var_matrix(Ntrain, B,0,1, name="c")
    else :
...
def print_Tree() :
    ...
    training_accuracy = (curMdl.objective_value / Ntrain ) * 100
...
def classification() :
    ...
    sizeSamples = [Ntrain, len(aMatrix) - Ntrain]
...
if (corrHeatmap) :
    ...
    Jlargest = featureScores.nlargest(J,'Score')
    plt.show()
